...
grouped = df.groupby(['virus', 'disease'])['uniquely_mapped_reads'].sum().reset_index()

# Create a pivot table with viruses as rows and disease status as columns
pivot = grouped.pivot(index='virus', columns='disease', values='uniquely_mapped_reads').fillna(0)
...
melted = pd.melt(pivot_reset, id_vars='virus', value_vars=pivot.columns,
                 var_name='disease', value_name='uniquely_mapped_reads')
...
ax = sns.barplot(x="virus", y="uniquely_mapped_reads", hue="disease", data=melted)
...
plt.savefig("../results/virus_only_uniquereads_diseasevscontrol.png", dpi=300)
