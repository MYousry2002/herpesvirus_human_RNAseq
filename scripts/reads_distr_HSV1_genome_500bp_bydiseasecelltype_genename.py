...
BIN_SIZE = 500  # changed from 1000 to 500
...
OUT_CSV = "../results/HSV1_KOS_read_distribution_500bp_bins.csv"
OUT_PNG = "../results/HSV1_KOS_read_distribution_heatmap_genename_groupedby_disease_celltype_500bp.png"
...
tick_labels = [f"{(i * BIN_SIZE) // 1000}kb" if i % 20 == 0 else "" for i in tick_positions]  # every 10kb = 20 bins
    ...
    for line in gtf:
        ...
        for field in attributes.strip().split(";"):
            if "gene_name" in field or "Name" in field:
                ...
                break
        gene_regions.append((start, end, gene_name))

# Add to plot
for start, end, gene_name in gene_regions:
    ax.axvspan(start / BIN_SIZE, end / BIN_SIZE, color="white", alpha=0.12)
    ax.text(
        (start + end) / (2 * BIN_SIZE), -1,
        gene_name,
        ha="center", va="bottom",
