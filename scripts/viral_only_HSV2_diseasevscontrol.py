...
hsv1_df = df[df['virus'] == 'HSV2_StrainG'].copy()

# Ensure total_mapped_reads is numeric
hsv1_df['viral_only_reads'] = pd.to_numeric(hsv1_df['viral_only_reads'], errors='coerce')

# 2. Separate groups
group_control = hsv1_df[hsv1_df['disease'] == 'Control']['viral_only_reads']
group_disease = hsv1_df[hsv1_df['disease'] == 'Juvenile Dermatomyositis']['viral_only_reads']
...
# Final formatting
plt.title("HSV2_StrainG: Control vs. Juvenile Dermatomyositis")
plt.xlabel("")
plt.ylabel("Total Mapped Reads")
plt.tight_layout()

# Save and show
plt.savefig("../results/viral_only_HSV2_StrainG_control_vs_disease.png", dpi=300)
plt.show()