...
grouped = df.groupby(['virus', 'treatment_status'])['total_mapped_reads'].sum().reset_index()

# Create a pivot table with viruses as rows and disease status as columns
pivot = grouped.pivot(index='virus', columns='treatment_status', values='total_mapped_reads').fillna(0)
...
melted = pd.melt(pivot_reset, id_vars='virus', value_vars=pivot.columns,
                 var_name='treatment_status', value_name='total_mapped_reads')
...
ax = sns.barplot(x="virus", y="total_mapped_reads", hue="treatment_status", data=melted)

# Add titles and labels
plt.title("Total Mapped Reads per Virus by Treatment Status")
plt.xlabel("Virus")
...
plt.savefig("../results/virus_only_totalreads_treatmentstatus.png", dpi=300)
